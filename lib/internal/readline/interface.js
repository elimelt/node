 ...
 * This is the default separator set. Consumers may override it per-instance
 * via the `lineSeparator` option accepted by `createInterface()`.
 */
const defaultLineEnding = /\r?\n|\r(?!\n)|\u2028|\u2029/g;

// Characters that require escaping when embedding a literal string inside
// a regular expression pattern.
const kRegExpSpecialChars = /[.*+?^${}()|[\]\\]/g;

function escapeRegExp(s) {
  return StringPrototypeReplace(s, kRegExpSpecialChars, '\\$&');
}
...
function compileLineSeparator(sep) {
  ...
  if (ArrayIsArray(sep)) {
    ...
    // Longest-first so that e.g. '\r\n' is preferred over '\r' alone.
    const sorted = ArrayPrototypeSort(
      ArrayPrototypeSlice(sep),
      (a, b) => b.length - a.length,
    );
    const alternation = ArrayPrototypeJoin(
      ArrayPrototypeMap(sorted, escapeRegExp),
      '|',
    );
...
const kCrlfCompatible = Symbol('_crlfCompatible');
...
function InterfaceConstructor(input, output, completer, terminal) {
  ...
  if (lineSeparator !== undefined) {
    ...
    // `crlfDelay` only affects the default separator — a custom separator
    // fully specifies what counts as a line break, so the time-based
    // cross-chunk `\r\n` collapsing logic is skipped.
    this[kCrlfCompatible] = false;
  } else {
    this[kLineEnding] = undefined;
    this[kCrlfCompatible] = true;
  }
...
class Interface extends InterfaceConstructor {
  ...
  [kNormalWrite](b) {
    ...
    const lineEnding = this[kLineEnding] ?? defaultLineEnding;
    const crlfCompatible = this[kCrlfCompatible];
    ...
    if (newPartContainsEnding !== null) {
      ...
      this[kSawReturnAt] = crlfCompatible && StringPrototypeEndsWith(string, '\r') ?
        DateNow() :
        0;
